...
const ModifyImagesUploader = ({ reviewimageSet }: PropsType) => {
  ...
  useEffect(() => {
    if (reviewimageSet) {
      const setData = reviewimageSet.map((image) => image.url);
      setImages(setData);
      setPreviewImg(setData);
    }
  }, [reviewimageSet]);
...
export default ModifyImagesUploader;
